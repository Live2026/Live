...
- **Langues** : tout nouveau texte d'interface passe par les fichiers de traduction (`app/lib/l10n/app_fr.arb`, référence, et `app_en.arb`) et s'affiche avec `context.t.cle` ; jamais de texte écrit en dur dans un nouvel écran. Régénérer avec `flutter gen-l10n` (automatique à la compilation). Langues traduites : français, anglais ; les autres langues proposées retombent sur le français.
