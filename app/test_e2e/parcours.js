...
(async () => {
  ...
  try {
    ...
    const TOUR = [
      ...
      ['abonnes/moi', 'relations_menu', async () => { await bouton('Plus d’options pour Merveille K.'); }],
      ['bloques', 'bloques'],
      ['carte', 'carte_logements'],
      ['carte?espace=services', 'carte_pros'],
      ['recherche', 'recherche_corrigee', async () => { await saisir(0, 'climatiser'); await p.waitForTimeout(800); }],
      ['produit/p2', 'variantes', async () => { await bouton('Acheter', { exact: true }); await bouton('M', { exact: true }); }],
      ...
      ['explorer', 'explorer_outils', async () => { await defiler(40); }],
      ['immo', 'immo_sejours', async () => { await defiler(8); }],
      ['moi', 'moi_createur', async () => { await defiler(8); }],
      ['parametres', 'parametres_pays', async () => { await bouton('Pays et ville'); }],
      ...
      ['directs', 'directs'],
      ['direct/d1', 'direct'],
      ['direct/d1', 'direct_cadeaux', async () => { await bouton('Cadeau'); await bouton('Couronne'); }],
      ['direct/d3', 'direct_visite'],
      ['direct/lancer', 'direct_lancer'],
      ['studio', 'studio'],
      ['fans/kimbembe', 'fans'],
      ['sejours', 'sejours'],
      ['sejour/s1', 'sejour'],
      ['live-plus', 'live_plus'],
      ['publicite', 'publicite'],
      ['live-pro/offres', 'offres_pro'],
      ['livraison/LV-00482', 'livraison'],
