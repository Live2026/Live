...
6. **Démarrage sur ordinateur** : la marque et ses quatre promesses (argent protégé, vérification, Mobile Money, Live IA) à gauche, le formulaire à droite dans une colonne de 480 px au plus. Révision du 26/09/2026 : le formulaire est une carte blanche de 440 px, à hauteur de son contenu (pas toute la hauteur), centrée dans la moitié droite, posée sur une image de fond plein écran (`app/assets/images/fond_demarrage.jpg`, dégradé sombre à gauche pour la lisibilité ; sans image, le fond animé). Le numéro se saisit comme sur WhatsApp : drapeau et indicatif dans le champ, liste des six pays qui s'ouvre sous le champ (plus de panneau en bas), numéro groupé pendant la frappe et opérateur Mobile Money reconnu dessous (`ChampTelephone`). Aucune bibliothèque externe : `intl_phone_number_input` dépend de libphonenumber natif, mal pris en charge sur le web, et `international_phone_text_field` 0.0.3 est trop jeune ; six pays suffisent.
