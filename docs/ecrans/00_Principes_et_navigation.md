...
6. **Démarrage sur ordinateur** : la marque et ses quatre promesses (argent protégé, vérification, Mobile Money, Live IA) à gauche, le formulaire à droite dans une colonne de 480 px au plus. Révision du 26/09/2026, sur le modèle de WhatsApp Web : plus de présentation ni d'image à gauche. Fond crème très clair, logo en haut à gauche, une seule carte centrée (560 px, bord fin, arrondi de 24 px) à hauteur de son contenu ; sous la carte, l'autre chemin (« Déjà un compte Live ? Se connecter » ou « Pas encore de compte ? Créer un compte »), la promesse de protection et les liens Conditions · Confidentialité · Aide. Sur ordinateur, la connexion propose aussi le code QR (`/connexion/qr`, E-AUTH-09) : le téléphone déjà connecté le scanne depuis Paramètres › Appareils connectés › Connecter un appareil ; pas de SMS à payer, et le code change toutes les 30 secondes. Nuance par rapport à WhatsApp : le pays et le numéro restent dans un seul champ (`ChampTelephone` : drapeau et indicatif dans le champ, liste des six pays sous le champ, numéro groupé, opérateur reconnu), car six pays ne justifient ni liste séparée ni recherche. Aucune bibliothèque externe : `intl_phone_number_input` dépend de libphonenumber natif, mal pris en charge sur le web, et `international_phone_text_field` 0.0.3 est trop jeune.
