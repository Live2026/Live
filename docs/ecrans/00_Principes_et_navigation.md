...
6. **Démarrage sur ordinateur** : la marque et ses quatre promesses (argent protégé, vérification, Mobile Money, Live IA) à gauche, le formulaire à droite dans une colonne de 480 px au plus. Révision du 26/09/2026, sur le modèle de WhatsApp (application et Web), simple et propre :
   - **Accueil (`/bienvenue`), même page sur téléphone et ordinateur, sans carte** : comme l'accueil de WhatsApp, un cercle serré de dessins au trait (`MotifLive`, les espaces de Live, d'une seule couleur, le bleu de Live) dans le haut de la page, le logo assis au centre dans la même couleur (`LogoMotif`) ; au-delà du cercle, un voile très léger des mêmes dessins jusqu'aux bords. Sous le cercle, « Bienvenue sur Live » et une phrase ; en bas et centrés, « Commencer », « Découvrir sans compte », « Déjà un compte ? Se connecter », puis Conditions · Confidentialité · Aide. Sur ordinateur, le logo en haut à gauche. Le motif est dessiné : rien à télécharger, net à toutes les tailles.
   - **En-têtes du démarrage** (téléphone et ordinateur) : comme WhatsApp, titre centré dans le bleu de Live et une phrase, sans icône colorée ; pendant l'inscription, une petite mention « Étape 1 sur 5 » au-dessus (repère utile sur cinq étapes).
   - **Numéro (`/telephone`, `/connexion`)**, sur le modèle de « Saisissez votre numéro » de WhatsApp : le pays sur sa propre ligne (drapeau, nom ; la liste des six pays s'ouvre juste dessous), puis le numéro précédé de l'indicatif, groupé pendant la frappe, l'opérateur Mobile Money reconnu dessous ; bouton « Suivant ». Avant l'envoi du code, confirmation « Vous avez saisi le numéro : … Est-il correct ? » (Modifier / OK) : un SMS envoyé à un numéro mal saisi est perdu et payé. Les cases de consentement restent (règle 12). Aucune bibliothèque externe.
   - **Code SMS** : titre « Vérifiez votre numéro » sans icône, comme WhatsApp.
   - **Autres pages du démarrage sur ordinateur** : le motif, plus discret, en fond ; une carte centrée de 560 px à hauteur de son contenu, bord fin ; dans la carte (`DansCarte`), bouton compact centré et cases compactes. Sous la carte, l'autre chemin (se connecter ou créer un compte), la promesse de protection et les liens.
